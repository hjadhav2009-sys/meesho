...
export function ProductImage({ src, alt, size = "md", showBadge = true, mappingId, showDebug = false }: ProductImageProps) {
  const [state, setState] = useState<ProductImageState>(getInitialProductImageState(src));
  const validSrc = isLoadableImageUrl(src) ? src : null;
  const hasSource = Boolean(validSrc);
  const stateText = productImageStateText(state, hasSource);

  useEffect(() => {
    setState(getInitialProductImageState(src));
    if (src && !validSrc && mappingId) {
      void markProductImageBrokenAction(mappingId);
    }
  }, [mappingId, src, validSrc]);

  const badge =
    state === "loaded"
      ? { label: "Image mapped", className: "bg-teal-50 text-teal-700 ring-teal-200" }
      : state === "broken"
        ? { label: stateText, className: "bg-rose-50 text-rose-700 ring-rose-200" }
  ...
  return (
    ...
    >
      ...
      {showDebug && state === "broken" && src ? (
        <span className="absolute inset-x-2 bottom-2 truncate rounded bg-white/90 px-2 py-1 text-[10px] font-medium text-slate-600">
          {src}
        </span>
      ) : null}
    </div>
